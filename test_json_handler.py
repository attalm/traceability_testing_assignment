...
@requirement("REQ-104")
def test_update_json(another_json_handler, another_file):
    """
    Test updating JSON data in a file.
    """
    data = {"test": "data"}
    another_json_handler.write_json(data, another_file)
    another_json_handler.update_json("test", "new data", another_file)
    updated_data = another_json_handler.read_json(another_file)
